...
// arcoin: optional setting to unlock wallet for block minting only;
    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %sAR %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
...
// arcoin: total coins staked (non-spendable until maturity)
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        if (pcoin->nTime > nSpendTime)
            continue;  // arcoin: timestamp must not exceed spend time
		...
		
    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // arcoin: sub-cent change is moved to fee
...
// arcoin: create coin stake transaction
...
// arcoin: check 'spent' consistency between wallet and txindex
// arcoin: fix wallet spent state according to txindex
...
// arcoin: disable transaction (only for coinstake)
