...
unsigned int nStakeMaxAge = 60 * 60 * 8;	// 8 hours
unsigned int nStakeTargetSpacing = 60;		// 60 second block time

int64 nChainStartTime = 1405404834;
...
const string strMessageMagic = "ARcoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // arcoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++)
    {
        ...
        // arcoin: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // arcoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// arcoin: find block wanted by given orphan block
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if(nHeight >= 1 && nHeight < 50) nSubsidy = 1 * COIN;
    else if(nHeight > 50 && nHeight <= 100) nSubsidy = 10 * COIN;
    else if(nHeight > 100 && nHeight <= 800) nSubsidy = 500 * COIN;
    else if(nHeight > 800 && nHeight <= 1300) nSubsidy = 400 * COIN;
    else if(nHeight > 1300 && nHeight <= 1700) nSubsidy = 300 * COIN;
    else if(nHeight > 1700 && nHeight <= 2050) nSubsidy = 200 * COIN;
    else if(nHeight > 2050 && nHeight <= 2650) nSubsidy = 125 * COIN;
    else if(nHeight > 2650 && nHeight <= 3650) nSubsidy = 75 * COIN;
    else if(nHeight > 3650 && nHeight <= 4250) nSubsidy = 125 * COIN;
    else if(nHeight > 4250 && nHeight <= 10100) nSubsidy = 200 * COIN;
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    nRewardCoinYear = MAX_AR_PROOF_OF_STAKE;
...
// arcoin: find last block index up to pindex
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake) 
{
    ...
    // arcoin: target change every block
    // arcoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // arcoin: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // arcoin: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // arcoin: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // arcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in arcoin
    bool fStrictPayToScriptHash = true; // Always active in arcoin
    ...
    // arcoin: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;
    if (!txdb.WriteBlockIndex(CDiskBlockIndex(pindex)))
        return error("Connect() : WriteBlockIndex for pindex failed");

    // arcoin: fees are not collected by miners as in bitcoin
    // arcoin: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// arcoin: total coin age spent in transaction, in the unit of coin-days.
...
// arcoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // arcoin: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // arcoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit(pindexNew->nHeight)))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // arcoin: record proof-of-stake hash value
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // arcoin: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // arcoin: coinbase output should be empty if proof-of-stake block
    ...
    if ((nTime >= CUTOFF_POW_TIME) && (IsProofOfWork()))
        return DoS(100, error("CheckBlock() : Proof of work (%f AR) at t=%d on or after %d.\n",
...
bool CBlock::AcceptBlock()
{
    ...
    // arcoin: check that the block satisfies synchronized checkpoint
    ...
    // arcoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // arcoin: verify hash target and signature of coinstake tx
    if (pblock->IsProofOfStake())
    {
        uint256 hashProofOfStake = 0;
        if (!CheckProofOfStake(pblock->vtx[1], pblock->nBits, hashProofOfStake, fIsInitialDownload))
        {
    ...
    // arcoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // arcoin: getblocks may not obtain the ancestor block rejected
    ...
    // arcoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "arcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "14 July 2014 - I am The Wolf of WORLD";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0x797ce7deeb143909f4c8ac485cfb6af1611fbe01d4022484076a72e3ccc68c3a"));
        ...
        // arcoin: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint((!fTestNet ? hashGenesisBlock : hashGenesisBlockTestNet)))
            return error("LoadBlockIndex() : failed to init sync checkpoint");
    }

    // arcoin: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // arcoin: should not enter safe mode for longer invalid chain
    // arcoin: if sync-checkpoint is too old do not enter safe mode
    ...
    // arcoin: if detected invalid checkpoint enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // arcoin: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // arcoin: record my external IP reported by peer
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // arcoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // arcoin: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    // arcoin: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // arcoin: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // arcoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
                if (!pblock->SignBlock(*pwalletMain))
                    continue;
                printf("CPUMINER: A proof-of-stake block has been found! %s\n", pblock->GetHash().ToString().c_str());
